...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", required=True, help="Select the folder with json files."
    )
    ...
    for k, v in l[0]["results"].items():
        if (
            v["metric"] == "Mutual Information"
            or v["metric"] == "Spearman corr. difference"
        ):
            continue
        ...
        if v["metric"] == "Vendi":
            ...
            values = [
                abs(l[i]["results"][k]["value"]) for i in range(1, len(l))
            ]  #  remove reference
            index_min = (
                min(enumerate(values), key=lambda x: x[1])[0] + 1
            )  # +1 because the reference is the first column
