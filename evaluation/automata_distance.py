...
def jaccard(distrib1, distrib2):
    ...
    return 1 - len(true_words.intersection(learned_words)) / len(
        true_words.union(learned_words)
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compute the distances between learned automata."
    )
    parser.add_argument(
        "--input", nargs=2, required=True, help="Select the automata directories."
    )
    ...
    files = set(
        [os.path.split(p)[1] for p in dir_list1 if p.endswith("-language.json")]
    ).intersection(
        [os.path.split(p)[1] for p in dir_list2 if p.endswith("-language.json")]
    )
