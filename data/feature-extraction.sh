...
if [ -f "data.pcap" ]; then
    ...
    tshark -r data.pcap -2 -T fields -E header=y -E separator=, -E aggregator=\& -e tcp.payload $FLAGS "tcp.len > 0" | sed '/^[0-f]*,*$/d' | sed 's/\([0-f]\{50\}\)[0-f]*,/\1,/' > payload_tcp.csv
    echo "UDP payload types extraction"
    tshark -r data.pcap -2 -T fields -E header=y -E separator=, -E aggregator=\& -e udp.payload $FLAGS "udp.length > 0" | sed '/^[0-f]*,*$/d' | sed 's/\([0-f]\{50\}\)[0-f]*,/\1,/' > payload_udp.csv
else
