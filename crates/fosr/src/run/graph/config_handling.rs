...
pub fn handle_config_changes(
    ...
    if needs_update && let Some(config_content) = &configuration_file_state.config_file_content {
        ...
        let config_result =
            std::panic::catch_unwind(|| fosr_lib::network::import_network(config_content));
