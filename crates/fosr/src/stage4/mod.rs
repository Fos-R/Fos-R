...
pub trait Stage4: Clone + std::marker::Send + 'static {
...
#[derive(Debug, Clone)]
pub struct DummyStage4 {}

impl Stage4 for DummyStage4 {
...
fn handle_packets(
    ...
    loop {
        ...
        if now > next_timeout_check {
            ...
            for p in flows.iter() {
                if p.timestamps.last().unwrap() > &timeout {
                    stage4.close_session(&p.flow.get_flow_id());
                }
            }
        ...
        if let Some((recv_packet, _addr)) = received_data {
            ...
            if stage4.is_packet_relevant(recv_packet.get_flags()) {
                let mut flows = current_flows.lock().unwrap();
...
pub fn start(stage4: impl Stage4, incoming_flows: HashMap<Protocol, Receiver<Packets>>) {
    log::trace!("Start S4");
    ...
    for (proto, rx_s4) in incoming_flows.into_iter() {
        ...
        let stage4 = stage4.clone();

        join_handles.push(
            builder
                .spawn(move || {
                    handle_packets(stage4, proto, rx, tx, current_flows);
                })
                .unwrap(),
        );
