...
use chrono_tz::Tz;
use fosr_lib::{
    models, stage1, stage2, stage3, stage3::tadam::TadamGenerator, stage4, stats, stats::Target,
};
...
pub fn generate(
    ...
    if let Some(config) = profile {
        model = model.with_string_network(&config).unwrap();
    }
...
fn run_single_thread(
    ...
    let stats = Arc::new(stats::Stats::new(stats::Target::None));
