...
use crate::topo::sub_topology::{SubTopology, SubTopologyOrInternet, SubTopologyNode};
use crate::utils;
...
struct TopologyTreeNode {
    ...
    children: Vec<TopologyTreeNode>,
    depth: usize,
}

impl TopologyTreeNode {
    fn into_subtopology_list(self) -> Vec<SubTopologyOrInternet> {
        self.children.into_iter().map(|c| c.into_subtopology_list()).flatten().chain(std::iter::once(self.value)).collect()
    }

    fn new_root(value: SubTopologyOrInternet) -> Self {
        Self {
            value: value,
    ...
    fn add_child(&mut self, mut child: SubTopologyOrInternet, rng: &mut impl Rng) {
        ...
        let child = TopologyTreeNode { value: child,
        children: vec![],
        assigned: HashSet::new(),
        depth: self.depth + 1 };
    ...
    // pub fn add_child(&mut self, child: TopologyTreeNode) {
    //     self.children.push(child);
    // }
...
//     // topology_trees: Vec<TopologyTreeNode>,
//     // /// Global variables for tree generation
//     // root: Option<TopologyTreeNode>,
// }
...
//     }
    ...
    pub fn generate_topology(
        ...
        // Self::assign_interco_addresses(&mut topology, topo_generation_parameters.tree_depth)?; // TODO: en même temps que la construction de l’arbre

        // Build one tree over the selected sub-topologies, exactly `tree_depth` levels when possible
        generate_tree(topology, topo_generation_parameters.tree_depth, !topo_generation_parameters.no_internet_access).map(TopologyTreeNode::into_subtopology_list)
    ...
    fn generate_tree(mut nodes: Vec<SubTopology>, max_depth: usize, internet_access: bool) -> Result<TopologyTreeNode, String> {
        ...
        let mut root = if internet_access {
            // Internet access: the root is the Internet
            TopologyTreeNode::new_root(SubTopologyOrInternet::Internet)
        } else {
            // no Internet access: the root is a subtopo
            // we know there is at least one node
            TopologyTreeNode::new_root(SubTopologyOrInternet::SubTopo(nodes.pop().unwrap()))
        };

        let mut rng = Pcg32::seed_from_u64(12345);
        let available_parents: Vec<usize> = vec![0];

        while let Some(node) = nodes.pop() {
            let child = SubTopologyOrInternet::SubTopo(node);
            root.add_child(child, &mut rng);
            // let parent_index = rng.gen_range(0..available_parents.len());
...
//         let mut subtree: Option<TopologyTreeNode> = None;
//         for node in nodes[..chain_len].iter().rev() {
//             let mut parent = TopologyTreeNode::new(node);
//             match subtree {
//                 Some(child) => parent.add_child(child),
//                 None => {
//                     for leaf in leaves {
//                         parent.add_child(TopologyTreeNode::new(leaf));
//                     }
//                 }
//             }
//             subtree = Some(parent);
...
//         Some(root)
    ...
    // fn save_tree_if_unique(&mut self, root: TopologyTreeNode) {
    //     let exists = self
    //         .topology_trees
    //         .iter()
    ...
    // fn same_trees(&self, root1: &TopologyTreeNode, root2: &TopologyTreeNode) -> bool {
    ...
    // pub fn get_generated_topologies_trees(&self) -> &Vec<TopologyTreeNode> {
    //     &self.topology_trees
    // }
...
//         let mut tree = TopologyTreeNode::new("root");
//         tree.add_child(TopologyTreeNode::new("child1"));
//         tree.add_child(TopologyTreeNode::new("child2"));
...
//     fn print_tree_compact(node: &TopologyTreeNode, indent: usize) {
//         let prefix = "  ".repeat(indent);
