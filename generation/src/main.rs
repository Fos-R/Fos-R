...
struct Args {
    ...
    #[arg(
        short,
        long,
        help = "Network interface to send synthetic network packets"
    )]
    ...
    #[arg(
        ...
        long,
        default_value_t = 10,
        help = "Number of flows to generate"
    )]
    nb_flows: i32,
...
fn main() {
    ...
    let nb_automata =
        s2.import_automata_from_dir(Path::new(&args.models_path).join("tas").to_str().unwrap());
