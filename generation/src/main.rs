...
fn main() {
    ...
    gen_threads.push(
        builder
            .spawn(move || {
                log::trace!("Start S0");
                let s0 = stage0::UniformGenerator::new(seed, online, 2, 10);
                for ts in s0 {
    ...
    let patterns = Arc::new(
        flowchronicle::PatternSet::from_file(
            Path::new(&args.models)
                .join("patterns.json")
                .to_str()
                .unwrap(),
        )
            ...
            builder
                .spawn(move || {
                    ...
                    while let Ok(ts) = rx_s1.recv() {
                        ...
                        if online {
                            // only keep relevant flows
                            flows
                                .filter(|f| {
                                    let data = f.data.get_data();
                                    local_interfaces.contains(&data.src_ip)
                                        || local_interfaces.contains(&data.dst_ip)
                                })
    ...
    let automata_library = Arc::new(tadam::AutomataLibrary::from_dir(
        Path::new(&args.models).join("tas").to_str().unwrap(),
    ));
                ...
                .spawn(move || {
                    ...
                    while let Ok(flow) = rx_s2.recv() {
                        ...
                        match flow.data {
                            Flow::TCP(data) => {
                                tx_s2_tcp
                                    .send(s2.generate_tcp_packets_info(SeededData {
                                        seed: flow.seed,
                                        data,
                                    }))
                                    .unwrap();
                            ...
                            Flow::UDP(data) => {
                                tx_s2_udp
                                    .send(s2.generate_udp_packets_info(SeededData {
                                        seed: flow.seed,
                                        data,
                                    }))
                                    .unwrap();
                            ...
                            Flow::ICMP(data) => {
                                tx_s2_icmp
                                    .send(s2.generate_icmp_packets_info(SeededData {
                                        seed: flow.seed,
                                        data,
                                    }))
                                    .unwrap();
                        ...
                        .spawn(move || {
                            ...
                            while let Ok(headers) = rx_s3_tcp.recv() {
                                ...
                                let flow_packets = s3.generate_tcp_packets(headers);
                                {
                                    ...
                                    *bc +=
                                        flow_packets.data.flow.get_data().fwd_total_payload_length
                                            + flow_packets
                                                .data
                                                .flow
                                                .get_data()
                                                .bwd_total_payload_length;
                                }
                                // dbg!(&flow_packets);
                                if online {
            ...
            builder
                .spawn(move || {
                    ...
                    if let Ok(packets_record) = rx_pcap.recv() {
                        ...
                        stage3::pcap_export(packets_record, &outfile, false)
                            .expect("Error during pcap export!");
                        while let Ok(packets_record) = rx_pcap.recv() {
                            log::trace!("Saving into {}", outfile);
                            stage3::pcap_export(packets_record, &outfile, true)
                                .expect("Error during pcap export!");
                        }
                    }
                })
