...
fn close_session(sockets: &Arc<Mutex<Vec<SocketSessions>>>, fid: &FlowId) {
    ...
    for sessions in sockets.iter_mut() {
        if let Some(pos) = sessions.flowids.iter().position(|f| f == fid) {
            ...
            clear_iptables(sessions.port).unwrap();
        } else {
...
fn setup_iptables(port: u16) -> io::Result<()> {
    // First rule: drop packets generated by the OS
    let drop_rule = Command::new("iptables")
        .args(&[
            "-t",
            "mangle",
            "-A",
    ...
    if drop_rule.status.success() {
        log::info!("iptables DROP rule set for port {}", port);
    } else {
        log::info!("iptables DROP rule not set for port {}", port);
    }

    // Second rule: set TTL of packets generated by Fos-R
    let ttl_rule = Command::new("iptables")
        .args(&[
            "-t",
            "mangle",
            "-A",
...
fn clear_iptables(port: u16) -> io::Result<()> {
    // First rule: Remove DROP rule
    let drop_rule = Command::new("iptables")
        .args(&[
            "-t",
            "mangle",
    ...
    if drop_rule.status.success() {
        log::info!("iptables DROP rule cleared for port {}", port);
    } else {
        log::info!("iptables DROP rule not cleared for port {}", port);
    }

    // Second rule: Remove TTL rule
    let ttl_rule = Command::new("iptables")
        .args(&[
            "-t",
            "mangle",
                ...
                while let Ok(flow) = incoming_flows.recv() {
                    ...
                    if let Flow::TCP(tcp_flow) = &flow.data.flow {
                        ...
                        if !found {
                            ...
                            setup_iptables(tcp_flow.src_port).unwrap();
                        }
                    } else {
